...
/* @author Anfilov. I do not agree that we shall ignore all UPDATE queries. We can execute
            ...
            try {
                ...
                if (e instanceof SQLIntegrityConstraintViolationException) {
                    if (e.getMessage().toLowerCase().contains("unique constraint")) {
                        throw new FedException(new Throwable("Record already exists with this primary key."));
                    } else if(e.getMessage().toLowerCase().contains("integrity constraint")) {
    ...
    private static void disableAllReferentialConstraints(String query, Statement statement) {
	 ...
	 * Sets true if the Create query has to be deployed on first 2 DBs, that
        ...
        String queryForDB1 = buildPartitionedQueryForDB1(query, createFewerPartitionsThanDBs);
        String queryForDB2 = buildPartitionedQueryForDB2(query, createFewerPartitionsThanDBs);
        String queryForDB3 = buildPartitionedQueryForDB3(query, createFewerPartitionsThanDBs);
     ...
     */
        ...
        try {
            ...
            CustomLogger.log(Level.INFO,
                    "Sending to:" + ConnectionConstants.CONNECTION_1_SID + ": " + queryForDB1.replaceAll("  ", " ")
                            .replaceAll("\r\n", " ").replaceAll("\t", " "));
        ...
        try {
            ...
            CustomLogger.log(Level.INFO,
                    "Sending to: " + ConnectionConstants.CONNECTION_2_SID + ": " + queryForDB2.replaceAll("  ", " ")
                            .replaceAll("\r\n", " ").replaceAll("\t", " "));
        ...
        try {
            if (!createFewerPartitionsThanDBs) {
                ...
                CustomLogger.log(Level.INFO,
                        "Sending to: " + ConnectionConstants.CONNECTION_3_SID + ": " + queryForDB3.replaceAll("  ", " ")
                                .replaceAll("\r\n", " ").replaceAll("\t", " "));
    ...
    private static boolean createFewerPartitionsThanDBs(String query) {
        ...
        String columnName = query
                .substring(query.indexOf("HORIZONTAL (") + "HORIZONTAL (".length(), query.lastIndexOf("("));
        // Fetching range
    ...
    private static String buildPartitionedQueryForDB1(String query, boolean createFewerPartitionsThanDBs) {
    ...
    private static String buildPartitionedQueryForDB2(String query, boolean createFewerPartitionsThanDBs) {
        if (createFewerPartitionsThanDBs) {
            return "";
        }
        ...
        String columnName = query
                .substring(query.indexOf("HORIZONTAL (") + "HORIZONTAL (".length(), query.lastIndexOf("("));
        String lowerRange = query
                .substring(query.indexOf(columnName + "(") + (columnName.length() + 1), query.lastIndexOf(","));
        String upperRange = query
                .substring(query.indexOf(lowerRange + ",") + (lowerRange + ",").length(), query.lastIndexOf("))"));

        // Appends constraint name
        basicQuery.append(tableName + "_" + columnName + "_HORIZ check (");
        basicQuery.append(columnName);
        basicQuery.append(" between " + (lowerRange.contains("\'") ? lowerRange : (Integer.parseInt(lowerRange) + 1))
                + " and " + upperRange);
        basicQuery.append(")");
    ...
    private static String buildPartitionedQueryForDB3(String query, boolean createFewerPartitionsThanDBs) {
        StringBuffer executableQuery = new StringBuffer();
        StringBuffer basicQuery = new StringBuffer(query.substring(0, query.indexOf("HORIZONTAL")));
